...
const Header = (props: HeaderProps) => {
  ...
  const { setSorting } = useArticleService();

  const navigate = useNavigate();
  const refValue = useRef<HTMLInputElement | null>(null);

  const searchHandler = () => {
    if (refValue.current && refValue.current.value) {
      const searchVal = refValue.current.value.trim();
      setSorting(`?$filter=contains(title,%27${searchVal}%27)`);
      navigate(`/search`);
    }
  };
  ...
  return (
    <header
      ...
      className={`bg-[image:var(--image-url)] bg-cover bg-no-repeat ${
        variantSmall ? "pt-12 pb-12" : "pt-32 pb-32"
      }`}
    >
      <div className={`w-[760px] m-auto flex flex-col items-center`}>
        <h1 className={`pb-4 ${variantSmall && "text-white text-xl"}`}>
          {title}
        </h1>
        ...
        >
          ...
          <IconButton
            type="button"
            sx={[
              { p: "10px", bgcolor: "#437EF7", borderRadius: 1 },
